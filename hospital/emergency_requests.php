...
if (isset($_GET['delete_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM request_responses WHERE request_id=?");
    $stmt->bind_param("i", $delete_id);
    $stmt->execute();
    $stmt->close();

    // Then delete the emergency request
    $stmt = $conn->prepare("DELETE FROM emergency_requests WHERE request_id=?");
    $stmt->bind_param("i", $delete_id);
    $stmt->execute();
    $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['publish_request'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO emergency_requests 
        (institution_id, blood_group, message, latitude, longitude, status, units_needed, units_donated, created_at) 
        VALUES (?, ?, ?, ?, ?, 'pending', ?, 0, NOW())");
    $stmt->bind_param("issddi", $institution_id, $blood_group, $message, $hospital_lat, $hospital_lng, $units_needed);
    $stmt->execute();
...
$all_requests = $conn->prepare("SELECT er.request_id, er.blood_group, er.message, er.status, er.created_at, er.units_needed,
        ...
        <div class="gradient-card mb-4">
            ...
            <form method="POST" class="row g-3 mt-2">
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control" name="units_needed" min="1"
