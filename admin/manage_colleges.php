...
if ($search_query !== '') {
    // Perform search
    $stmt = $conn->prepare("SELECT * FROM institutions WHERE name LIKE CONCAT('%', ?, '%') AND type = 'college' ORDER BY institution_id DESC");
    ...
    $stmt = $conn->prepare("SELECT * FROM institutions WHERE type = 'college' ORDER BY institution_id DESC LIMIT 20");
    $stmt->execute();
    $result = $stmt->get_result();
    $colleges = $result->fetch_all(MYSQLI_ASSOC);
    $searched = false;
    ...
    <div class="container-fluid">
        ...
        <div class="table-responsive table-wrapper">
                    ...
                    <?php if (count($colleges) > 0): ?>
                        <?php foreach ($colleges as $index => $college): ?>
                            ...
                            <tr>
                                ...
                                <td><?= htmlspecialchars($college['email']) ?></td>
                                <td><?= htmlspecialchars($college['type']) ?></td>
                                <td><?= htmlspecialchars($college['status']) ?></td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="no-results">
                                <?= $search_query !== '' ? "No Colleges found for '{$search_query}'." : "No Colleges found." ?>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            ...
            <?php foreach ($colleges as $college): ?>
                <div class="modal fade" id="viewModal<?= $college['institution_id'] ?>" tabindex="-1"
                    aria-labelledby="viewModalLabel<?= $college['institution_id'] ?>" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered modal-lg">
                        <div class="modal-content">
                            <!-- Modal Header -->
                            <div class="modal-header bg-primary text-white">
                                <h5 class="modal-title" id="viewModalLabel<?= $college['institution_id'] ?>">College Details</h5>
                            ...
                            <div class="modal-body">
                                <table class="table table-bordered">
                                    ...
                                    <tr>
                                        <th>Address</th>
                                        <td><?= htmlspecialchars($college['address']) ?></td>
                                    </tr>
                                </table>
                            </div>
